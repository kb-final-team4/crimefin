<template>
  <v-container
    fluid
    class="d-flex flex-column align-center justify-center h-100 main-container"
  >
      ...
      <v-col width="50%" height="80%">
        <v-card>
          <v-card-title class="mx-auto" align="center">
            <div id="logo">
              <v-img src="../assets/CRIME (2).png" width="50%"></v-img>
            </div>
          </v-card-title>
          ...
          <v-card-text>
            <v-form @submit.prevent="login" class="mx-auto" max-width="80%">
              ...
              <v-text-field
                ...
                type="password"
              ></v-text-field>
              <v-btn type="submit" class="mt-4 login-btn" outlined block
                >로그인</v-btn
              >
            </v-form>
          </v-card-text>
...
export default {
  ...
  methods: {
    async changePassword() {
      ...
      try {
        const response = await axios.put("http://13.125.225.44:9999/user/pwd", {
          memberId,
          newPassword,
          confirmPassword,
        });
    ...
    async findUserId() {
      axios
        .get("http://13.125.225.44:9999/user", {
          params: {
            name: this.name,
            phone: this.phone,
          },
    ...
    async requestVerificationCode() {
      try {
        const response = await axios.post(
          "http://13.125.225.44:9999/user/sendNum",
          {
    ...
    async confirmVerficationCode() {
      axios
        .get("http://13.125.225.44:9999/user/checkNum", {
          params: {
    ...
    login() {
      ...
      var url = "http://13.125.225.44:9999/user/login";
      var data = {
        memberId: this.memberId,
